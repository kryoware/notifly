...
Figma file key `BTqrcTY3MPDY5WeDng5dzi` remains a visual reference.
`ui/theme/Color.kt` is the authoritative local token file. Regenerate it from
the four original primary seeds with `python tools/generate_colors.py` after
