...
- `docs/MD_ICONS.md` — icons are Material Symbols vector drawables
  (`composeResources/drawable/symbol_*.xml`), generated by
  `python tools/download_symbols.py`. Add a name to `NAMES` and rerun; never
  hand-draw or use `Icons.Default`.
- `docs/MD3_LIST.md` — list pattern for any list with bulk actions: long-press
  starts selection, tap toggles while selecting, contextual `TopAppBar`
  (close X, count, actions, `surfaceVariant` container), selected row tint via
  `animateColorAsState`, avatar `Crossfade`s to `symbol_check_circle`, swipe
  actions via `SwipeToDismissBox` with `customActions` for accessibility.
  `TransactionsScreen` / `TransactionRow` is the reference implementation.
  The guide's sample `Color.Gray`/`Color.White` do not apply here — colour rule
  above wins.
